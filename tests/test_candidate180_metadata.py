...
class Candidate180MetadataTests(unittest.TestCase):
    ...
    def test_bundle_metadata_pending_candidate193_preserves_candidate192_card(self):
        info = plistlib.loads((ROOT / 'kext/Info.plist').read_bytes())
        self.assertEqual(info['CFBundleShortVersionString'], '1.0.193')
        self.assertEqual(info['CFBundleVersion'], '1.0.193')
