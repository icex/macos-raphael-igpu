// Bounded sustained hardware decode qualification. Run only under supervision.
// CLI: probe h264|hevc hw|sw encoderRegistry frames width height decoderRegistry(0=auto)
...
static int width = 1280, height = 720, frameCount = 3;
static NSMutableIndexSet *seenFrames;

static void emit(NSString *phase, NSDictionary *fields) {
    if ([phase isEqualToString:@"encode-begin"] || [phase isEqualToString:@"encode-end"] ||
        [phase isEqualToString:@"encode-callback"] || [phase hasPrefix:@"decode-submit-"]) return;
    if ([phase isEqualToString:@"decode-callback"] && ([fields[@"frame"] intValue] % 60)) return;
...
static uint8_t luma(int x, int y, int frame) {
    return 32 + 40*(x >= width/2) + 80*(y >= height/2) + 5*(frame % 12);
}
...
static void decodedFrame(void *context, void *source, OSStatus status,
    ...
    @autoreleasepool {
        ...
        if(frame<0 || frame>=frameCount || [seenFrames containsIndex:frame]) errors++;
        else [seenFrames addIndex:frame];
...
int main(int argc, const char **argv) {
    @autoreleasepool {
        ...
        seenFrames = [NSMutableIndexSet indexSet];
        ...
        if (argc != 8 || (strcmp(argv[1], "h264") && strcmp(argv[1], "hevc")) ||
            (strcmp(argv[2], "hw") && strcmp(argv[2], "sw"))) {
            fprintf(stderr,"usage: probe h264|hevc hw|sw registryID (0 for sw), or --list\n");
            return 2;
        }
        BOOL decodeHardware = YES;
        BOOL hardware = !strcmp(argv[2], "hw");
        frameCount=atoi(argv[4]); width=atoi(argv[5]); height=atoi(argv[6]);
        uint64_t decoderRegistry=strtoull(argv[7],NULL,0);
        if(frameCount<1 || frameCount>6000 || width<64 || width>3840 || height<64 || height>2160 || (width%2) || (height%2)) return 2;
        ...
        emit(@"invalidate-end", @{});
        // Require and verify actual hardware decoding; software fallback cannot pass.
        if(samples.count) {
            VTDecompressionSessionRef decoder=NULL;
            VTDecompressionOutputCallbackRecord callback={decodedFrame,NULL};
            NSMutableDictionary *decodeSpec=[@{
                (__bridge id)kVTVideoDecoderSpecification_EnableHardwareAcceleratedVideoDecoder:@(decodeHardware),
                (__bridge id)kVTVideoDecoderSpecification_RequireHardwareAcceleratedVideoDecoder:@(decodeHardware)} mutableCopy];
            if(decoderRegistry) decodeSpec[(__bridge id)kVTVideoDecoderSpecification_RequiredDecoderGPURegistryID]=@(decoderRegistry);
            emit(@"hardware-decode-support", @{@"supported":@(VTIsHardwareDecodeSupported(codec))});
            emit(@"decode-create-begin", @{@"hardware_requested":@(decodeHardware), @"registry_id":@(registry)});
            ...
            if(!status && decoder) {
                CFTypeRef usingHardware=NULL;
                OSStatus query=VTSessionCopyProperty(decoder,kVTDecompressionPropertyKey_UsingHardwareAcceleratedVideoDecoder,NULL,&usingHardware);
                BOOL selected=usingHardware && CFEqual(usingHardware,kCFBooleanTrue);
                emit(@"selected-decoder", @{@"status":@(query), @"hardware":@(selected)});
                if(decodeHardware ? (query || !selected) : ((query && query != kVTPropertyNotSupportedErr) || selected)) errors++;
                if(usingHardware) CFRelease(usingHardware);
                for(id sample in samples) {
                    emit(@"decode-submit-begin", @{});
                    status=VTDecompressionSessionDecodeFrame(decoder,(__bridge CMSampleBufferRef)sample,0,NULL,NULL);
                    emit(@"decode-submit-end", @{@"status":@(status)});
                    if(status) errors++;
                }
        ...
        BOOL passed=!errors && samples.count==frameCount && decoded==frameCount && seenFrames.count==frameCount && checked && maxDifference<=8;
        emit(@"result", @{@"passed":@(passed), @"errors":@(errors), @"encoded_frames":@(samples.count),
             @"decoded_frames":@(decoded), @"unique_frames":@(seenFrames.count), @"width":@(width), @"height":@(height), @"luma_values_checked":@(checked),
