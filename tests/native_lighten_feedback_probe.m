// Installed QuartzCore lighten fragment with same-target feedback, checked after one pass.
...
int main(int argc, const char **argv) { @autoreleasepool {
    ...
    "fragment half4 base(V in [[stage_in]]) {uint2 p=uint2(in.p.xy);return half4(half((p.x/20)%17)/16,half((p.y/20)%13)/12,half((p.x/20+p.y/20)%11)/10,1);}\n"
    ...
    for (unsigned storage=0;storage<3;storage++) for (unsigned barrier=0;barrier<4;barrier++) { @autoreleasepool {
        ...
        [enc setRenderPipelineState:base];
        ...
        [[NSData dataWithBytes:out.contents length:row*height] writeToFile:[NSString stringWithFormat:@"/var/tmp/native-lighten-%d-%u-%u.bgra",argc>2?atoi(argv[2]):0,storage,barrier] atomically:YES]; unsigned bad=0,firstX=0,firstY=0;float maxError=0;
        for(unsigned y=303;y<803;y++)for(unsigned x=105;x<325;x++) {
            float expect[3]={quantize((x/20%17)/16.f),quantize((y/20%13)/12.f),quantize(((x/20+y/20)%11)/10.f)};
    ...
    emit(@{@"phase":@"done",@"cases":@12,@"failures":@(failures)});return failures?1:0;
}}
