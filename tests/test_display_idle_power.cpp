...
int main() {
 for (unsigned fault=0;fault<6;fault++) {
  ...
  auto write=[&](uint32_t a,uint32_t v) {
   ...
   else { assert(a==Message); assert(v==2 || v==0x12 || v==7);
    response=fault==2?0:1;
    if(v==2) argument=fault==3?0xdead:0x625300;
    if(v==0x12 && fault==4) response=0xff;
    if(v==7) { assert(argument==1000); if(fault==5) response=0xff; }
   }
  };
  auto r=exitIdle(read,write,[](){},0x625300,1000);
  if(fault==0) { assert(r.wakeResponse==1); assert(writes.size()==9); assert(writes[4].second==0); }
  if(fault==1) assert(writes.empty());
  if(fault==2 || fault==3) { assert(writes.size()==3); assert(r.wakeResponse==0); }
  if(fault==4) { assert(r.wakeResponse==0xff); assert(writes.size()==6); assert(r.dcfResponse==0); }
  if(fault==0) { assert(r.dcfResponse==1); assert(r.dcfMHz==1000); }
  if(fault==5) { assert(r.dcfResponse==0xff); assert(r.dcfMHz==0); }
 }
}
