...
@end
int main(int argc,const char**argv){@autoreleasepool{alarm(240);
 ...
 NSDate*until=[NSDate dateWithTimeIntervalSinceNow:20];while(until.timeIntervalSinceNow>0){NSEvent*e=[NSApp nextEventMatchingMask:NSEventMaskAny untilDate:[NSDate dateWithTimeIntervalSinceNow:.05] inMode:NSDefaultRunLoopMode dequeue:YES];if(e)[NSApp sendEvent:e];phase++; if(phase%20==0) {
 double t=phase*.013;
 ...
 } [back.contentView setNeedsDisplay:YES];[NSApp updateWindows];} [w orderOut:nil];[back orderOut:nil];printf("ENDURANCE_DONE frames=%d\n",phase);fflush(stdout);return 0;}}
