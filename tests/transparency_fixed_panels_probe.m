// Bounded foreground capture variant: keep panels visible above their backdrop.
...
int main(int argc,const char**argv){@autoreleasepool{alarm(80);
 ...
 NSDate*until=[NSDate dateWithTimeIntervalSinceNow:20];while(until.timeIntervalSinceNow>0){NSEvent*e=[NSApp nextEventMatchingMask:NSEventMaskAny untilDate:[NSDate dateWithTimeIntervalSinceNow:.05] inMode:NSDefaultRunLoopMode dequeue:YES];if(e)[NSApp sendEvent:e];phase++;[back.contentView setNeedsDisplay:YES];[NSApp updateWindows];} [w orderOut:nil];[back orderOut:nil];return 0;}}
