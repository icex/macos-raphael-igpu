...
static bool batch(id<MTLDevice> dev, unsigned round, uint64_t *peak) { @autoreleasepool {
    const NSUInteger bytes=16*1024*1024, words=bytes/4, sets=4;
    ...
    *peak=dev.currentAllocatedSize;
    ...
    // Observe GPU completion before this batch releases its buffers.
...
int main(int argc,const char **argv) { @autoreleasepool {
    ...
    if(!dev) return 2;
    const unsigned rounds=4, warmup=1;
    const uint64_t tolerance=4*1024*1024;
    uint64_t baseline=dev.currentAllocatedSize, maxAfter=0, maxPeak=0;
    emit(@{@"phase":@"begin",@"registry_id":@(dev.registryID),@"pid":@(getpid()),@"initial_allocated":@(baseline),@"rss":@(rss()),@"rounds":@(rounds),@"warmup":@(warmup),@"live_resource_bytes":@(192*1024*1024),@"settle_tolerance_bytes":@(tolerance)});
    for(unsigned round=0;round<warmup+rounds;round++) {
        ...
        if(round>=warmup) {
            ...
            if(after>baseline+tolerance || peak<baseline+128*1024*1024) {
    ...
    emit(@{@"phase":@"done",@"passed":@YES,@"rounds":@(rounds),@"warmup":@(warmup),@"baseline":@(baseline),@"max_after":@(maxAfter),@"max_peak":@(maxPeak),@"measured_values_checked":@(4ULL*16*1024*1024),@"pid":@(getpid()),@"scope":@"process-local currentAllocatedSize; managed/private buffer copies only"});
    return 0;
}}
