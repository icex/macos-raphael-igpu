...
class RecoveryLeaseProductionTests(unittest.TestCase):
    ...
    def test_vmm_disable_reaches_native_path_before_vmm_ownership(self):
        wrapper = self._function(
            "static void wrapVmmSetAlloc", "static void probeRlc")
        self.assertIn(
            "FunctionCast(wrapVmmSetAlloc, orgVmmSetAlloc)(self, enable);", wrapper)
        self.assertNotIn("early disable", wrapper)
