// Bounded buffer GPU-address reuse and longer reclamation probe.
// Reused addresses demonstrate recycling, not page-table invalidation or global VA freedom.
...
static bool batch(id<MTLDevice> dev, unsigned round, uint64_t *peak) { @autoreleasepool {
    ...
    for(unsigned s=0;s<sets;s++) {
        ...
        for(id<MTLBuffer> b in @[input,intermediate,output]) {
            uint64_t address=b.gpuAddress;
            if(!address || address>UINT64_MAX-bytes) return false;
            for(NSDictionary *prior in addresses) {
                uint64_t a=[prior[@"address"] unsignedLongLongValue];
                if(address<a+bytes && a<address+bytes) return false;
            }
            [addresses addObject:@{@"address":@(address),@"length":@(bytes),@"storage":@(b.storageMode)}];
        }
    ...
    *peak=dev.currentAllocatedSize;
    ...
    emit(@{@"phase":@"batch",@"round":@(round),@"values_checked":@(words*sets),@"bad":@(bad),@"peak_allocated":@(*peak),@"addresses":addresses});
    return bad==0;
}}
int main(int argc,const char **argv) { @autoreleasepool {
    signal(SIGALRM,expired); alarm(900);
    uint64_t wanted=argc>1?strtoull(argv[1],NULL,0):0;
    id<MTLDevice> dev=nil;
    for(id<MTLDevice> d in MTLCopyAllDevices()) if(d.registryID==wanted) dev=d;
    if(!dev) return 2;
    const unsigned rounds=argc>2?(unsigned)strtoul(argv[2],NULL,0):512, warmup=3;
    if(rounds<1 || rounds>512) return 2;
    ...
    emit(@{@"phase":@"done",@"passed":@YES,@"rounds":@(rounds),@"warmup":@(warmup),@"baseline":@(baseline),@"max_after":@(maxAfter),@"max_peak":@(maxPeak),@"measured_values_checked":@((uint64_t)rounds*4*1024*1024),@"pid":@(getpid()),@"scope":@"GPU address recycling and process-local allocation; no page-table invalidation proof"});
    return 0;
}}
