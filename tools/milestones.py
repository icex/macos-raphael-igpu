...
DESCR = {
    ...
    "xk": "clear the state the host driver left in the graphics core. Walking the MEC queues after the KIQ timeout finds eight active HQD selectors all reporting the same ring at 0xFFBFEA0000 -- outside this iGPU's 0xf400000000-0xf41fffffff carveout, and exactly the address in the latched VM fault. That is amdgpu's KIQ, still mapped in the MEC from before the device was handed to vfio-pci, pointing at memory that no longer exists; CP_MEC_ME1_HEADER_DUMP reads 0xdef1def1, so MEC1 has never fetched a packet. Same shape as the stale PSP ring x7 destroys -- this GPU is never reset. Upstream clears it with one register: gfx_v10_0_cp_compute_enable writes CP_MEC_CNTL = MEC_ME1_HALT|MEC_ME2_HALT and then 0. Also ensures RLC_CNTL.RLC_ENABLE_F32 (already 1 here) and clears the VM fault latch so a new fault is attributable.",
    "xl": "accept the RLC firmware autoload on BOOTLOAD_COMPLETE alone. GC HW_INIT is where ttl_hw_init fails on a freshly reset GPU: _gc_check_ucode_loaded_10_1 polls _gc_fw_autoload_is_completed for 50 ms, never gets a yes, asserts (GC event 0xc0100206), and _gc_check_register_equal_ext then times out too. The predicate reads RLC_STAT (gc_reg_offset 0x4c04 base_idx 1, cached at ttl+0xb38) and RLC_RLCS_BOOTLOAD_STATUS (0x4e8d base_idx 1, at ttl+0xb3c), and demands BOOTLOAD_COMPLETE *and* RLC_STAT == exactly 0x25 (RLC_BUSY|RLC_GPM_BUSY|RLC_THREAD_0_BUSY). Upstream's gfx_v10_0_wait_for_rlc_autoload_complete asks only for BOOTLOAD_COMPLETE plus CP_STAT == 0; the RLC_STAT clause is Apple's addition and is a liveness sample, not a completion latch. Here BOOTLOAD_STATUS reads 0xc0000001 (complete) while RLC_STAT reads 0, so whether the window ever catches 0x25 is a race -- the same binary reached the accelerator on one host boot and fails on the next. Falls back to upstream's condition and logs all three registers.",
    "xj": "diagnostic only: trace the accelerator's hardware power-up chain. AMDGraphicsAccelerator::powerUpHW only reaches AMDHWMemory::enableAllocations after hardware->powerUp() and initializeHardwareRegisters() both succeed, and AMDHardware::powerUp needs powerUpHWEngines and startHWEngines. The accelerator's own progress bitfield reports 0 for all of them, which is why the VRAM pools are never initialised at all. Logs the return of each.",
    "xi": "tell PowerPlay it is unsupported instead of letting it power the GPU back down. AmdPowerPlayHelper::powerUp calls handleCriticalError(\"PowerUp Failed. Shut back down.\") and then vtable+0x198 (powerDown) whenever PPLIB init fails while [this+0x28f8] is 1 -- which is why the accelerator attaches and then has 0 bytes of VRAM by the time WindowServer submits. With the SMU on a dummy back end PowerPlay really is unsupported, and Apple has a path for that: isSupported() false makes powerUp log \"SKIP: Not Supported\", return 0xe00002c7 without entering PPLIB at all, and skip both handleCriticalError and powerDown. The controller already treats that as a warning and continues.",
    "xh": "give the accelerator's two memory pools a range that is not inverted. With TTL up, the aperture corrected and PowerPlay skipped, the first command buffer still page-faults on \"0 free memory remaining\". AMDRadeonX6000_AMDHWMemory::initVRAMInfo is where that heap gets its numbers; this logs total/reserved/free and the two size fields it validates, so the zero can be identified instead of guessed. Adds AMDRadeonX6000 as a third patched kext.",
    "xg": "take the framebuffer aperture from the GFXHUB copy. TTL is up and the accelerator attaches, then the first command buffer page-faults: GPUCAP reports FB Base 0x100000000 == FB Top, a zero-wide range, so the VRAM allocator has 0 bytes. AmdAsicInfoNavi2::populateXGmiConfig reads MMHUB (dword indices 0x1a86c/0x1a86d/0x1a857), which on this part is simply not programmed: they read 0x100/0/0. The GFXHUB copy is -- GC base 0x1260 + the gc_10_3 offsets 0x16fc/0x16fd/0x16e7 = 0x295c/0x295d/0x2947 -- and reads 0xf400/0xf41f/0x840, i.e. 0xF400000000-0xF41FFFFFFF, exactly the 512M the host kernel reports for this iGPU. Overwrites the cached fields only when the GFXHUB pair describes a non-empty range.",
...
BITS = {"m1": 1, "m2": 2, "m3": 4, "m4": 8, "m5": 16, "m6": 32, "m7": 64, "d1": 128,
        "r1": 256, "p1": 512, "x1": 2048, "x2": 4096, "x3": 8192, "x4": 16384, "x5": 32768, "x6": 65536, "x7": 131072, "x8": 262144, "x9": 524288, "xa": 1048576, "xb": 2097152, "xc": 4194304, "xd": 8388608, "xe": 16777216, "xf": 33554432, "xg": 67108864, "xh": 134217728, "xi": 268435456, "xj": 536870912, "xk": 1073741824, "xl": 2147483648}
