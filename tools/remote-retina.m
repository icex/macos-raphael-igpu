...
int main(int argc,const char **argv) { @autoreleasepool {
    ...
    // Prefer an existing public mode; no topology change is needed in that case.
    if(selectMode(1920,1080,3840,2160)){pump(0.5);bool ok=retina(inventory());report(@"configured",ok);return ok?0:3;}
    ...
    bool selected=applied&&removed&&selectMode(1920,1080,3840,2160);pump(0.5);
    bool ok=selected&&retina(inventory());
    if(!ok&&removed)selectMode([before[0][@"width"] unsignedIntegerValue],[before[0][@"height"] unsignedIntegerValue],
                             [before[0][@"pixel_width"] unsignedIntegerValue],[before[0][@"pixel_height"] unsignedIntegerValue]);
