...
def preflight(args, pins: dict, worktree: Path, vm: Path) -> dict:
    ...
    power = Path(f"/sys/bus/pci/devices/{pins['gpu_bdf']}/power/control")
    try:
        pinned = power.read_text().strip() == "on"
    except OSError as error:
        raise CycleError(f"cannot verify GPU power/control: {error}") from error
    if not pinned:
        raise CycleError("GPU power/control must be pinned on before reset")
...
def prepare_and_run(args, facts: dict, worktree: Path, vm: Path, run_id: str) -> dict:
    ...
    try:
        ...
        if not isinstance(verdict, dict):
            raise ValueError("wrapper result must be an object")
...
def main() -> int:
    ...
    if outcome["returncode"]:
        return outcome["returncode"] if outcome["returncode"] > 0 else 1
    if outcome["verdict"].get("verdict") in (None, "UNKNOWN", "INVALID", "WRAPPER_FAILURE"):
        return 2
    return 0
