...
## Regression audit — the Metal path is intact (2026-09-14)

Prompted by the concern that the working Metal desktop had regressed, the live deployment was
compared field-by-field against the candidate-230 state that passed the 96-case copy matrix. It
is **byte-identical**: same normalized `boot-args`, same VBIOS in `ATY,bin_image` (connector ROM
`3c6977ee…`), same kext set, same 5 kernel patches, and the deployed kext is **1.0.230** — the
proven build. **No part of the GPU/Metal configuration changed.** Version 1.0.231 exists only in
the `candidate-231` worktree (the closed route-(a) experiment) and is not deployed.

What *was* changed during the display investigation is guest-side and display-only, not Metal:

- Both WindowServer display preference plists were moved aside to `/var/root/wsprefs-aside/` in
  the guest (chasing the missing virtual display). **Pending restore.**
- Screen Sharing was reconfigured via `kickstart` (legacy VNC toggled on, then off). **Pending
  restore.**

Neither touches the accelerator, the corrections, or the ROM, so Metal acceleration is expected
to be exactly as proven. This has **not** been re-exercised on hardware since the route-(a)
revert; the copy matrix should be re-run once to confirm before relying on it.
...
| GPU | `0000:7b:00.0`, bound to `vfio-pci`, `power/control=on` |
| Last harness-recorded launch | **40** (candidate 230 repeat) |
| Later same-boot launches | supervised `vm-supervision.py` runs for the route-(a) `metal-079` experiment (`cycle.py` refused harness admission with `boot_already_used` before QEMU opened, so no ledger entry was consumed) |
| Last MODE2 reset receipt | `run/mode2-reset-64.json` |
| VM | stopped |
...
8. **Guest-side display state left modified during the display investigation (2026-09-14).** Two
   persistent changes inside the guest are **pending restore** and should be reverted before the
   guest is treated as a clean baseline: (a) both WindowServer display preference plists moved to
   `/var/root/wsprefs-aside/`; (b) Screen Sharing reconfigured via `kickstart` (legacy VNC on,
   then off). These are display/remote-session only and do not affect Metal (see the regression
   audit above), but they change guest boot behaviour.
