...
## Offline harness/driver audit update (2026-09-12)

The disproven candidate-202 engine skip-success, power-service bypass, and
wrong-ABI `wireSysMemory` hook were removed offline; candidate-199 cleanup and
cold-disable logic remain. Offline validation passed: EngineLifecycle and
RecoveryLease C++ fixtures, 24 route/recovery Python tests, 115 experiment
tests, 38 supervisor/worktree tests, wrapper tests, preflight, and Python
compilation. No new GPU run or authority was created. The current boot ledger
is exhausted and historically contaminated (`max_launches=16`, duplicate run
IDs, 16 rows); this is evidence only and was not edited. The remaining driver
blocker is the candidate-195 cold initialization boundary; native startup is
not claimed fixed. The required separate Astra review was attempted but no
completed review was available due service/thread limits.

The wrapper now uses ordinary admission, writes fsynced stdout/stderr and
`wrapper-result.json` artifacts, classifies non-JSON or launcher errors, derives
candidate/run identity from the embedded manifest, preserves exit status, and
uses the canonical repository status path. Offline wrapper regression tests:
**7 passed**; no new run authority or GPU exposure was created.

Candidate-195 offline TLS audit: panic registers and symbol offsets consistently
infer loaded base `0xffffff801c67c000`; RDI is the TLV descriptor, R15 is the
map pointer, and RIP is an invalid indirect target. Loaded base is not
independently logged, so this remains a bounded causal finding. Removed per-map
TLS correlation while retaining aggregate submission commit records, restored
the native VMM disable path and prior owned/ready condition, and corrected the
diagnostic log. Explicit-repository preflight passed (its other inputs still
come from `/home/bogdan/macos-vm`; the old default-source result is superseded).
Private audited-working-source build passed; artifact and full log are at
`/tmp/rgpu-audited-working-source-build-20260912/` and
`/tmp/rgpu-audited-working-source-build-20260912.log`. Manifest records
`source_clean=false`; no candidate authority, stage, hardware run, or ledger
entry was created. The built Mach-O contains no `__thread_vars`,
`__thread_bss`, `__thread_data`, or `__tlv_bootstrap`.
...
Review policy: use Luna medium for routine implementation and tests. After every
three actual GPU attempts, refresh this status and complete the required offline
Astra xhigh adversarial review before another hardware launch. The current batch
has reached that gate (at minimum candidates 199, 200, and 201); no completed
post-batch Astra review is recorded.

The continuous iteration workflow is documented in
`docs/superpowers/plans/2026-09-12-continuous-metal-iterations.md`. Candidate
202 must not be used: its skip-engine-success guard does not cover the observed
candidate-201 fault. Candidate 203 is not authorized for hardware until the
required review completes and its repair is revised against the exact RIP audit.
...
Candidate 201 is the latest useful driver observation. The exact X6000 load base
was `0xffffff7f9c122000` and the fault RIP was `0xffffff7f9c1830ad`, hence file
offset `0x610ad`. In the pinned KDK binary this is
`AMDGFX10ComputeRing::enable+0x21`, instruction `48 89 08` / `mov [rax],rcx`.
The panic registers were `RAX=0`, `RCX=0`, `RDI=0xffffff9062b6ea00`, and
`CR2=0`: the compute-ring object's member at `+0xb0` was null and Apple wrote
through it. `AMDHardware::powerUp+0x66` is a caller return address, not the RIP.

The exact KDK producer for ring `+0xb0` is
`AMDRTRing::allocateMemoryResources`: initialization first clears it at
`0x5f5f0`; successful allocation publishes `r15+8` at `0x5f86f`. Candidate 201
logged one experimental `wireSysMemory` null return before virtual-space
readiness, then the power-service failure branch was bypassed and PM4 startup
continued. The routed KDK method requires `self` plus five explicit arguments,
but the wrapper declares only `self` plus four; it shifts `options`, `task`, and
`flags` and cannot safely call native. This wrong-ABI/null-return route is the
leading producer-side cause, but the intermediate allocation return that leaves
ring `+0xb0` null was not captured, so the full causal link is not yet proven.

Commit `93c62a5` does not guard this fault. It checks the outer engine pointer,
engine vtable, and engine power-up slot, all of which were valid because execution
reached `PM4Engine::doStart` and the compute-ring virtual `enable` call. Its
null-engine check is also unreachable through `powerUpAll`, which already skips
null engine entries. Treating a null vtable or power-up slot as success would hide
corrupt/incomplete initialization. The minimum repair is to remove that
skip-as-success change, stop bypassing the platform power-service failure, and
remove the invalid `wireSysMemory` route; preserve and propagate native failures
until the real cold VMM/handler contract is repaired.

Candidate-201 teardown stopped QEMU and left the device accessible on `vfio-pci`,
but that alone does not authenticate graphics retirement or repeatable recovery.
The ledger contains additional candidate-201-era reservations/exposures whose
classification is not fully reconstructed here, so the attempt count is a lower
bound and launch authority remains closed.
...
Superseded by the exact-RIP audit above: the routed native member actually takes
five explicit arguments in addition to `self`, while `cf29208` captures only
four. Its native pass-through therefore has the wrong ABI, and the broad
`nullptr` return is a leading producer-side fault rather than a valid deferral.
Candidate 201 does not capture the intermediate allocation return, so a direct
wire-call-to-ring-null chain remains unproven.
...
Superseded diagnosis: this paragraph previously attributed the fault to the
outer engine pointer/vtable call in the engine loop. Exact-RIP resolution shows
the outer PM4 engine call succeeded and the fault is instead the null internal
compute-ring `+0xb0` resource described in the authoritative section above.
Candidate 201 remains an evidence fixture; commit `93c62a5` and candidate 202 do
not repair it.
