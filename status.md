...
### Native offline audit after cycle 014

The native audit confirms six ordinary-serial mangling sites (lines 2149, 2280, 2281, 2282, 2284, 2287) plus one malformed CR2 prefix; the checksummed CR2 terminal snapshot remains valid. `allocPhysical` completed 7,268 callbacks with 233 true and 7,035 false; this proves substantial rejection but does not distinguish capacity, fragmentation, ownership, or unreleased resources. VMID-2 roots and level-0 tables are coherent, while sampled leaves include `V=0`/`TF=1` without any observed VM fault (`status=0 addr=0`). The discriminating next test is one fresh authorized cycle with identical topology and a single small known-to-fit Metal submission, measuring exact allocation result/free-pool state, the submission's VM walk, fence completion, and CP stall state without changing PTEs or pool bounds. Full audit: `findings/research/report-native-cycle014.md`.

### Serial recheck after cycle 014 (2026-09-12)

A second pass over `serial.txt` and the checksum-backed CR2 records found no missed GPU fault. The strongest ordered markers remain `XJ: AMDGraphicsAccelerator::powerUpHW -> 1`, `waitForHwStamp(1..3)->1`, then `waitForHwStamp(4..7)->0`, with `VM_FAULT_STATUS=0 addr=0`. The apparent `Constraint not satisfied` and `TRM_EnrollmentPending` messages are Apple credential/transport noise adjacent to the allocator burst, not RaphaelGPU fault records. Allocation failures are real and repeated: requests of 0x37e000, 0x87c000, 0x2204000, and 0x2280000 bytes occur while free memory is below the request; the summary still reports `completed=7268 true=233 false=7035`. VM traces show repaired root `0x84b6f3000`, valid converted child/leaf samples, and some `V=0`/`TF=1` leaves without a nonzero fault status. The next small-submission test therefore remains the correct discriminator; no new root-cause claim is justified from the mangled lines.

## Metal-035 small probe audit (2026-09-11)

Run `302ff99b6dee4afb97a30f692fb3d275` on boot `1a97ba0a-f5d6-40cd-90e9-c8f5072e7010` used the final candidate-194 reseal and `small-metal` manifest. The guarded run reached engine startup, published an authenticated ACTIVE pool and VALID lifetime, and completed the first KIQ stamps (`waitForHwStamp(1..3) -> 1`); the next KIQ boundary failed (`waitForHwStamp(4) -> 0`, with later stamps 5..19 timing out despite bounded re-submissions). Verdict is `BASELINE_BLOCKED`, earliest failure `kiq`; the native probe did not produce a result.

Submission evidence reached `process=26/25/0`, `mappings=54/54/11`, `prepare=97/96/2`, `map=241/241/11`, `submit=359/359/0`. Backing allocation ended at `completed=334 true=67 false=267 dropped=263`; map phase ended `total=14 capacity=0 va=0 backing-pte=14 unknown=0 dropped=0/0/12/0`. Repeated snapshots reported `VM_FAULT_STATUS=0 addr=0`; CP reported `CP_CPC_STALLED_STAT1` changing to `0x210000` before the terminal KIQ failures.

Forced shutdown was required because guest identity transport failed (`request_sent=false`, `acpi_request_sent=true`). Recovery is schema 6 `incomplete`: `dequeued=1`, `dequeue_timeouts=8`, `forced_inactive=8`, `gfx_ring_clean=false`, and `gfx_retirement_confirmed=false`; PSP teardown completed and host state remained `vfio-pci`, `active_vm=false`, `device_accessible=true`, and `sleep_inhibited=true`. The GPU ledger contains **1 launch** for this boot (the finite budget is consumed); no retry or policy bypass is permitted.
